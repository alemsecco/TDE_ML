...
def add_lag_features(df: pd.DataFrame):
    # espera df com MesAno, Regiao e Consumo numérico
    df = df.copy()
    df['Consumo'] = pd.to_numeric(df['Consumo'], errors='coerce')
    # garante que MesAno está em formato datetime para ordenação
...
def preprocess(df: pd.DataFrame):
    # Remove linhas sem MesAno ou Regiao
    df = df.dropna(subset=['MesAno', 'Regiao']).copy()
    # adiciona características de defasagem temporal
    df = add_lag_features(df)

    # Analisa MesAno para extrair ano e mês
    df['MesAno'] = df['MesAno'].astype(str)
    df['Ano'] = df['MesAno'].str.slice(0,4).astype(int)
    df['Mes'] = df['MesAno'].str.slice(5,7).astype(int)

    # Variável alvo: Consumo
    df['Consumo'] = pd.to_numeric(df['Consumo'], errors='coerce')

    # Colunas de características: Ano, Mes, Regiao (one-hot), TEMPERATURA MEDIA, PRECIPITACAO, PRESSAO, VENTO
    features = []
    features += ['Ano', 'Mes']
    # inclui defasagens temporais se presentes
    ...
    # Transformador de colunas: imputa valores numéricos, codifica Regiao como one-hot
...
def time_series_compare(df: pd.DataFrame, n_splits=5):
    ...
    # converte MesAno para índice datetime para ordenação
    df_idx = df.copy()
    ...
    # Modelos para treinar: RandomForest e LightGBM (se disponível)
    models = [('rfr', RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=-1))]
    if LGB_AVAILABLE:
        models.append(('lgb', None))  # marcador de posição
    ...
    for name, estimator in models:
        ...
        for train_idx, test_idx in tss.split(X):
            ...
            if name == 'lgb' and LGB_AVAILABLE:
                # constrói dataset para lightgbm
...
def train_and_save(X, y, preprocessor, model='rfr', model_path=None):
    ...
    # Se o modelo é sensível à escala (mlp, knn), adiciona um scaler para características numéricas dentro do ColumnTransformer
    if model in ('mlp', 'knn'):
        # modifica o preprocessador para escalar características numéricas
        # encontra características numéricas dos transformadores do preprocessador (hack: recria)
        # Por simplicidade, vamos construir um novo ColumnTransformer aqui similar ao preprocess()
        ...
        # tenta extrair colunas do preprocessador original
        try:
